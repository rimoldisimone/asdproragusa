...
if (get_option_tree('blog_title') && is_home()) {
	...
	if(is_day()) : 
		$title = sprintf( __( '%s', 'Nevada' ), get_the_date() );
					
	elseif(is_month()) : 
		$title = sprintf( __( '%s', 'Nevada' ), get_the_date('F Y') );
					
	elseif(is_year()) : 
		$title = sprintf( __( '%s', 'Nevada' ), get_the_date('Y') );
					
	elseif(is_category()) :
		$title = sprintf( __( '%s', 'Nevada' ), single_cat_title( '', false ) );
		
	elseif(is_tag()) :
		$title = __( 'Tag Archives', 'Nevada' );
		
	elseif(!is_page() && ( !is_home() && !is_front_page() ) ) : 
		
		$title = __( 'Blog Archives', 'Nevada' );
