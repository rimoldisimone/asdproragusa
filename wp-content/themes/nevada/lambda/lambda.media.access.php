...
* prints media button
*
* This function will output a media button in your metabox.
* Needs to be wrapped in a container element such as <p> or <div>
* with corresponding text input
*
* @param	string $button_text - metabox media button text
* @param	string $uploader_title - media manager title
* @param	string $uploader_button - media manager insert media button text
* @param	string $uploader_type - file type accepted in media manager - valid values: "image", "video", "audio", or "application"
* @return	string - html a.upload-button
*
*/
function wpa_media_button($button_text = "choose a file", $uploader_title = "Site Files", $uploader_button = "choose a file", $uploader_type = ""){
    $button = sprintf('<a href="#" class="upload-button button" data-uploader_title="%s" data-uploader_button_text="%s" data-limit_type="%s">%s</a>', $uploader_title, $uploader_button, $uploader_type, $button_text);
    return $button;
}
 ...
 class WPAlchemy_MediaAccess
{
	...
	public function getButtonClass($groupname = null)
	{
		...
		return $this->button_class_name . '-' . $groupname . ' thickbox btn btn-info';
	}
