  ...
  <div class="option option-background-upload" id="<?php echo $value->item_id; ?>_div">
    ...
    <div class="section">
      <div class="element">
		...
		if ($handle = opendir($absolute_path_to_theme.'/images/footer/bg-textured/prev/')) {
			...
			while (false !== ($file = readdir($handle))) {
				if($file != '..' && $file != '.') {
					...
					echo '<div class="single_pattern">
							...
							<label class="label-for-check" for="'.$value->item_id.'_'.$count.'"><div class="pattern" style="background: url('.THEME_WEB_ROOT.'/images/footer/bg-textured/prev/'.$file.');"></div></label></div>';
					$count++;
				}
			}
