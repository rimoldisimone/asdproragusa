			...
			while (false !== ($file = readdir($handle))) {
				if($file != '..' && $file != '.') {
					...
					echo '<div class="single_pattern">
							...
							<label class="label-for-check" for="'.$value->item_id.'_'.$count.'"><div class="pattern" style="background: url('.THEME_WEB_ROOT.'/images/bg-textured/prev/'.$file.');"></div></label></div>';
					$count++;
				}
			}
