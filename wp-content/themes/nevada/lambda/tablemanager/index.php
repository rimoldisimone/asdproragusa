...
<div id="lambda-options-panel-title">
	...
	<h1><?php echo _e('Manage your Tables', 'Nevada')?></h1>
	<div class="clear"></div>
...
<table cellspacing="0" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<?php _e('List of created Tables', 'Nevada')?>
								
				</th>
			</tr>
            <tr>
            	<td> <?php _e('ID', 'Nevada'); ?> </td>
            	<td> <?php _e('Table Name', 'Nevada'); ?> </td>
				<td> <?php _e('Shortcode', 'Nevada'); ?> </td>
            	<td> <?php _e('Edit', 'Nevada'); ?> </td>
            	<td> <?php _e('Delete', 'Nevada'); ?> </td>
            </tr>
			</thead>
			<tbody>
      ...
      echo '<tr>
       ...
       <tr>
		   ...
		   <td colspan="6">
				...
				<label class="control-label"><?php _e('Enter Table Name', 'Nevada'); ?></label>
				
				<div class="controls">
				<input type="text" id="table_name" class="lambda_input" name="table_name" size="70" placeholder="<?php _e('Table Name', 'Nevada'); ?>" /><br />
				</div>
				
				<button type="submit" class="btn btn-success" value="<?php _e('Add new Table', 'Nevada'); ?>" /><i class="icon-plus icon-white"></i></button>
				
				<br />
				
				<span class="info"><?php _e('* Do not use spaces or special characters in the name. This Name is only for internal use!', 'Nevada'); ?></span>
...
if ( isset($_GET['add']) && $_GET['add'] == 'table' ) {	
	...
	if( $table_exists ) {
		...
		$tm_message = __( 'Table already exists' , 'Nevada' );
		...
		$tm_message = __( 'Table successfully added' , 'Nevada' );
...
if ( isset($_GET['delete']) ) {	
	...
	$tm_message = __('Table deleted', 'Nevada');
}
...
function lambda_table_edit() { ?>
	...
	<div class="lambda-opttitle">
            <div class="lambda-opttitle-pad settingstitle">
                <h1><?php echo _e('Table Manager', 'Nevada')?></h1>
                
				<input type="hidden" name="action" value="table_update" />
				<input type="hidden" name="page_options" value="<?php echo $tablename; ?>" />
				<input type="submit" class="lambda_save btn btn-success right" value="<?php _e('Save Settings', 'Nevada') ?>" />
	...
	<div id="table-items" class="tab-pane">
		...
		<div id="single-columns">
			...
			<label for="column_<?php echo $tablename.'_'.$key; ?>"><?php _e('Activate Column?', 'Nevada'); ?></label><br />
			
			<div class="lambda_row">
				
				<div class="btn-group" data-toggle="buttons-radio">
				
				<button value="<?php echo $tablename.'_'.$key; ?>_on" type="button" class="btn btn-mini btn-info <?php echo ($value['column_active'] == 'on') ? 'active' : 'inactive'; ?> radio_<?php echo ($value['column_active'] == 'on') ? 'active' : 'inactive'; ?>"><?php _e('on', 'Nevada'); ?></button>				
				<button value="<?php echo $tablename.'_'.$key; ?>_off" type="button" class="btn btn-mini btn-info <?php echo ($value['column_active'] == 'off') ? 'active' : 'inactive'; ?> radio_<?php echo ($value['column_active'] == 'off') ? 'active' : 'inactive'; ?>"><?php _e('off', 'Nevada'); ?></button>
			...
			<label for="column_<?php echo $tablename.'_'.$key; ?>"><?php _e('Column is featured?', 'Nevada'); ?></label><br />
			
			<div class="lambda_row">
				
				<div class="btn-group" data-toggle="buttons-radio">
				
				<button value="<?php echo $tablename.'_'.$key; ?>_yes" type="button" class="btn btn-mini btn-info <?php echo ($value['featured'] == 'yes') ? 'active' : 'inactive'; ?> radio_<?php echo ($value['featured'] == 'yes') ? 'active' : 'inactive'; ?>"><?php _e('yes', 'Nevada'); ?></button>				
				<button value="<?php echo $tablename.'_'.$key; ?>_no" type="button" class="btn btn-mini btn-info <?php echo ($value['featured'] == 'no') ? 'active' : 'inactive'; ?> radio_<?php echo ($value['featured'] == 'no') ? 'active' : 'inactive'; ?>"><?php _e('no', 'Nevada'); ?></button>
			...
			<label><?php _e('Column Headline', 'Nevada'); ?></label><br />
			...
			<label><?php _e('Column Price', 'Nevada'); ?></label><br />
			...
			<label><?php _e('Price Currency', 'Nevada'); ?></label><br />
			...
			<label><?php _e('Period', 'Nevada'); ?></label><br />
			...
			<hr />
					...
					<label><?php _e('Features', 'Nevada'); ?></label><br />
					...
					<button data-key="<?php echo $key; ?>" data-group="lambda_table_content" data-table="<?php echo $table; ?>" type="button" value="features_<?php echo $tablename.'_'.$key; ?>" class="add_column_feature right btn btn-mini btn-info" name="lambda_table_content[<?php echo $tablename; ?>]"><i class="icon-picture icon-white"></i> <?php _e('Add Row', 'Nevada'); ?></button>
			...
			<label><?php _e('Custom URL', 'Nevada'); ?></label><br />
			...
			<label><?php _e('Buttontext', 'Nevada'); ?></label><br />
			...
			<label><?php _e('HTML Content', 'Nevada'); ?></label><br />
