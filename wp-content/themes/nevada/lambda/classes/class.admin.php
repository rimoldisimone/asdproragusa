...
class OT_Admin
{
  ...
  function option_tree_notice_post_locked( $post_id ) 
  {
    ...
    $last_user_name = $last_user ? $last_user->display_name : __('Somebody', 'Nevada');
    $the_page = ( $_GET['page'] == 'option_tree' ) ? __('Theme Options', 'Nevada') : __('Settings', 'Nevada');
    
    $message = sprintf( __( 'Warning: %s is currently editing the %s.', 'Nevada'), esc_html( $last_user_name ), $the_page );
