...
<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
				...
				<section id="post-<?php the_ID(); ?>" <?php post_class('single'); ?>>
                    ...
                    <div class="entry-content clearfix">
						...
						<div class="entry-summary">					
							<?php the_content(); ?>
                            <?php wp_link_pages( array( 'before' => '<div class="page-link">' . __( 'Pages:', 'Nevada' ), 'after' => '</div>' ) ); ?>                           
                        </div>
				...
				<?php $tags_list = get_the_tag_list( '', ', ' );
                    	...
                    	<div class="post-ut">	
							...
							<span class="tag_links">
								<?php printf( __( '<span class="%1$s"></span> %2$s', 'Nevada' ), 'entry-utility-prep entry-utility-prep-tag-links', $tags_list ); ?>
							</span>
                ...
                <div id="nav-below" class="navigation loop-single clearfix">
					<div class="nav-previous"><?php previous_post_link( '%link', '<span class="meta-nav">' . _x( '&#8656;', 'Previous post link', 'Nevada' ) . '</span> %title' ); ?></div>
					<div class="nav-next"><?php next_post_link( '%link', '%title <span class="meta-nav">' . _x( '&#8658;', 'Next post link', 'Nevada' ) . '</span>' ); ?></div>
				</div><!-- #nav-below -->
